...
public class CategorySyncService {
	...
	public CategorySyncService(Context context){
		this.ctx = context;
	}
	 ...
	 * @return
	 */
	public Result syncCategory(String ticketId){
		String opUrl = LoginService.url + "/categoryItem";

		Map bodyData = new HashMap();// 发送的业务数据
		sharedPrefs = ctx.getSharedPreferences(Constants.PREF, Context.MODE_PRIVATE);
		String pref_syncTime = sharedPrefs.getString(Constants.PREF_CATEGORY_SYNCTIME, "2010-10-01 00:00:00");
		bodyData.put("syncTime", pref_syncTime);
		bodyData.put("categoryId", "");
		...
		try {
			...
			proxy.setBody(data);
			proxy.exec();
			Log.d(TAG,"分类信息_Recv:" + proxy.getResponseString());
			...
			if("0000".equals(code)){
				...
				editor.putString(Constants.PREF_CATEGORY_SYNCTIME,syncTime);
				editor.commit();
				...
				for (int i = 0; i < addAry.length(); i++) {
					...
					CategoryEntity category = new CategoryEntity(addItem);
					ctx.getContentResolver().insert(CategoryEntity.CONTENT_URI,
