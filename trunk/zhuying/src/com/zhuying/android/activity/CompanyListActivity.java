...
import com.zhuying.android.adapter.CompanyCursorAdapter;
 ...
 * 公司列表
 */
public class CompanyListActivity extends Activity implements IXListViewListener{
	public CompanyCursorAdapter adapter;
//	public PullToRefreshListView listView;
	private XListView mListView;
	public static List<ContentValues> list = new ArrayList<ContentValues>();
	...
	private static final String NAME = "name", COMPANY = "company",
			SORT_KEY = "sort_key", PARTYFACE = "partyface",
			PARTYID = "partyid";
	...
	private void doList(String searchKeyword) {
		mListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				/*String rid = "";
				Cursor c = managedQuery(CompanyEntity.CONTENT_URI, null,
						"_id = " + id, null, null);
				while (c.moveToFirst()) {
					rid = c.getString(19);
					break;
				...
				c.close();
				Uri uri = CompanyEntity.CONTENT_URI;
				Intent i = new Intent();
		...
		if (TextUtils.isEmpty(searchKeyword)) {
			selection = "partytype = '" + CompanyEntity.TYPE_COMPANY + "'";
			
			fromSearch = false;
		} else {
			selection = "partytype = '" + CompanyEntity.TYPE_COMPANY + "' and name like '%" + searchKeyword + "%'";
			
			fromSearch = true;
		}
		String sortOrder = "sort_key COLLATE LOCALIZED asc";
//		Cursor cursor = managedQuery(CompanyEntity.CONTENT_URI, null,
//				selection, null, sortOrder);
//		String[] from = { CompanyEntity.KEY_NAME };
//		int[] to = { R.id.name };
//		adapter = new CompanyCursorAdapter(this, R.layout.company_list_item,
//				cursor, from, to);
//		setListAdapter(adapter);
		
		asyncQuery = new MyAsyncQueryHandler(getContentResolver());
		asyncQuery.startQuery(1, null, CompanyEntity.CONTENT_URI, null, selection, null,sortOrder);
	}

	/*
	 * private void doSearch() { String[] selectionArgs = new String[] {
	 * searchtext }; Uri uri = Uri.parse("content://" + CompanyEntity.AUTHORITY
	 * + "/" + SearchManager.SUGGEST_URI_PATH_QUERY); listView = getListView();
	 * listView.setOnItemClickListener(new OnItemClickListener() { public void
	 * onItemClick(AdapterView<?> parent, View view, int position, long id) {
	 * Uri uri = ContentUris.withAppendedId(CompanyEntity.CONTENT_URI, id);
	 * startActivity(new Intent(Intent.ACTION_VIEW, uri)); } }); Cursor cursor =
	 * managedQuery(uri, null, null, selectionArgs,null); String[] from = new
	 * String[] { SearchManager.SUGGEST_COLUMN_TEXT_1,
	 * SearchManager.SUGGEST_COLUMN_TEXT_2 }; int[] to = { R.id.name };
	 * SimpleCursorAdapter a = new SimpleCursorAdapter(this,
	 * R.layout.company_list_item, cursor, from, to); setListAdapter(a); }
	 */
	...
	private class GetDataTask extends AsyncTask<Void, Void, String[]> {
		...
		protected String[] doInBackground(Void... params) {
			...
			contactService.syncCompany(ticketId);
			return null;*/
...
//				listView.setSelection(position + 1); // +1，解决listview addheaderview问题
	...
	private void setAdapter(List<ContentValues> list) {
		...
		if(fromSearch){
			emptyView = (LinearLayout) findViewById(R.id.empty_company_list_view_search);
		}else{
			emptyView = (LinearLayout) findViewById(R.id.empty_company_list_view);
		}
...
//		listView.setAdapter(madapter);
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				convertView = inflater
						.inflate(R.layout.company_list_item, null);
			...
			if (!TextUtils.isEmpty(partyFace)) {
				Cursor photoCursor = context.getContentResolver().query(
						...
						PhotoEntity.KEY_NAME + " = '" + partyFace + "'", null,
						null);
	...
	@Override
	public void onRefresh() {
		if(!NetworkStateUtil.checkNetworkInfo(CompanyListActivity.this)){
			mListView.stopRefresh();
		}else{
			new GetDataTask().execute();
		}
...
//		new UITask(CompanyListActivity.this) {
//			@Override
//			protected Result doInBackground(Void... arg0) {
//				String ticketId = sharedPrefs.getString(Constants.PREF_TICKETID, null);
//				NoticeSyncService service = new NoticeSyncService(CompanyListActivity.this);
//				Result r = service.syncNotice(ticketId);
//				return r;
//			}
//		}.execute();
		...
		new AsyncTask<Void, Void, String>() {
			...
			protected void onPostExecute(String result) {
				if(!StringUtil.isEmpty(result)){
					showNoticeDialog(CompanyListActivity.this,result);
				}
			};
		}.execute();
	}
